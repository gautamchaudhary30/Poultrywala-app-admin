...
const AproovedCompanies = () => {
  ...
  const filteredData = companiesData.filter((company) => {
    const matchesSearch =
      [company.business, company.district, company.name, company.email, company.phone].some((field) =>
        field.toLowerCase().includes(search.toLowerCase())
      );

    const matchesDistrict = districtFilter ? company.location.includes(districtFilter) : true;
    const matchesState = stateFilter ? company.location.includes(stateFilter) : true;

    return matchesSearch && matchesDistrict && matchesState && company.status === 'Approved';
  });

  return (
    <div className="p-6">
      <div className="flex items-center justify-between p-1 mb-4">
        <h2 className="text-3xl font-semibold dark:text-gray-100">Approved Companies</h2>
        {/* <button className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-800 active:bg-green-900 active:scale-95">
          <i className="ri-download-fill"></i> Export
        </button> */}
      </div>

      <p className="text-sm text-gray-500 mb-4">Manage and monitor approved company accounts</p>
      ...
      <div className="flex justify-between items-center mt-4 text-sm">
        <span className="space-x-2 text-left px-2 py-1 dark:text-gray-300">
          Showing {filteredData.length > 0 ? 1 : 0} to {filteredData.length} of {companiesData.length} results
        </span>
...
export default AproovedCompanies;
